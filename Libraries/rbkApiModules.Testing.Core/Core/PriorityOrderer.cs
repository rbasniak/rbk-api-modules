...
namespace Xunit.Priority
{
    public class PriorityOrderer : ITestCaseOrderer
    {
        ...
        public const string Assembly = "rbkApiModules.Testing.Core";
        ...
        {
            if (Assembly != GetType().Assembly.GetName().Name) 
            {
                throw new InvalidDataException($"Assemblies do not match: {Assembly} vs {GetType().Assembly.GetName().Name}");
            }

            if (Name != GetType().FullName)
            {
                throw new InvalidDataException($"TestCaseOrderer implementation name does not match: {Name} vs {GetType().FullName}");
            }
