...
public class UpdateDisciplines
{
    public class Request : AuthenticatedRequest, IRequest<CommandResponse>, ICommand<ChangeRequest>
    {
        ...
        public IEnumerable<IDomainEvent<ChangeRequest>> ExecuteOn(ChangeRequest entity)
        {
            ...
            {
                if (entity.Disciplines.None(x => x == disciplineId))
                {
                    results.Add(new DisciplineAddedToChangeRequest.V1(Identity.Username, ChangeRequestId, disciplineId));
                }
            }

            foreach (var disciplineId in entity.Disciplines)
            {
                if (DisciplineIds.None(x => x == disciplineId))
                {
                    results.Add(new DisciplineRemovedFromChangeRequest.V1(Identity.Username, ChangeRequestId, disciplineId));
                }
            }
