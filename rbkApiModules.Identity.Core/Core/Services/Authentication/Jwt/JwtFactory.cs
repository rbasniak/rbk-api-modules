...
public class JwtFactory : IJwtFactory
{
    ...
    public async Task<string> GenerateEncodedTokenAsync(string username, Dictionary<string, string[]> roles, JwtOptionsOverride jwtOptionsOverride, CancellationToken cancellationToken)
    {
        ...
        if (_authenticationOptions._allowTenantSwitching && !String.IsNullOrEmpty(currentTenant))
        {
            ...
            {
                var tenants = await _tenantsService.GetAllAsync(cancellationToken);
            ...
            {
                var tenants = await _usersService.GetAllowedTenantsAsync(username, cancellationToken);
        ...
        {
            throw new ExpectedInternalException("Token generated without authentication mode");
        }
