...
public static class AvatarGenerator
{
    ...
    public static string GenerateSvgAvatar(string fullName)
    {
        ...
        if (parts.Length == 1)
        {
            initials = parts.First()[0].ToString().ToUpper();
        }
        else
        {
            initials = parts.First()[0].ToString().ToUpper() + parts.Last()[0].ToString().ToUpper();
        }

        var svg = "<svg xmlns=\"http://www.w3.org/2000/svg\" width=\"256px\" height=\"256px\"><rect width=\"256\" height=\"256\" style=\"fill:{{COLOR}}\"/><text x=\"50%\" y=\"142.5px\" dominant-baseline=\"middle\" text-anchor=\"middle\" style=\"font-size:160px;fill:#fff;font-family:ArialMT, Arial\">{{INITIALS}}</text></svg>";
